...
typedef enum {
    LTE_STATUS_INIT = 0,
    LTE_STATUS_ACTIVATED = 1,
    LTE_STATUS_DEACTIVATED = 2
} LTE_Status;

static LTE_Status lte_status = LTE_STATUS_INIT;
static char lte_apn[256] = {0};
static char lte_pdp_type[256] = {"IP"};
...
static void requestRadioPower(void *data, size_t datalen, RIL_Token t)
{
    ...
    onOff = ((int *)data)[0];
    RLOGD("requestRadioPower:onOff=%d, sState=%d\n", onOff, sState);
...
static int syncGetLteNetAddr(int cid,char * addr)
{
	ATResponse *p_response = NULL;
	ATLine *p_cur;
	int skip;
	...
	err = at_send_command_singleline("AT+CGPADDR", "  +CGPADDR:",&p_response);//+CGPADDR: 1,"172.22.1.100"
	if(err < 0 || p_response->success == 0)
		goto error;
	...
	err = at_tok_nextstr(&line, &tmpptr);
	if ((err < 0) || ('\0' == *tmpptr))
		goto error;
...
static int syncGetGWNetmask(int cid,char * gw,char * netmask)
{
	ATResponse *p_response = NULL;
	ATLine *p_cur;
	int skip;
	...
	err = at_send_command_singleline("AT+RAWIP", "  +RAWIP:",&p_response);// +RAWIP: 1,"255.255.255.0","172.22.1.101"
	...
	RLOGD("gw:%s, netmask:%s", gw, netmask);
	at_response_free(p_response);
	p_response = NULL;
	return 0;
...
static int syncGetDns(int cid,char * dns1,char * dns2)
{
	ATResponse *p_response = NULL;
	ATLine *p_cur;
	int skip;
	...
	err = at_send_command_singleline("AT+CGPDNS=1", "  +CGPDNS:",&p_response);//  +CGPDNS: 1,"172.22.1.201","172.22.1.200"
...
int activateNetDev(char * dev)
{
	char shell[256];
	int ret = -1;

	sprintf(shell,"netcfg %s up dhcp",dev);
	ret = system(shell);
	RLOGD("%s returned %d\n",shell,ret);
...
int deactivateNetDev(char * dev)
{
	char shell[256];
	int ret = -1;

	sprintf(shell, "netcfg %s down", dev);
	ret = system(shell);
	RLOGD("%s returned %d\n",shell,ret);
...
static void checkCgact()
{
    ...
    if ((err != 0) || (0 == p_response->success)) goto error;
    ...
    if (srv_status == 0 && lte_status == LTE_STATUS_ACTIVATED) {
    	lte_status = LTE_STATUS_DEACTIVATED;
    	deactivateNetDev(PPP_TTY_PATH);
...
static void requestOrSendDataCallList(RIL_Token *t)
{
    ...
    for (p_cur = p_response->p_intermediates; p_cur != NULL;
        ...
        //ip address
        responses[i].addresses = alloca(32);
        err = syncGetLteNetAddr(cid, responses[i].addresses);
        if (err < 0)
        	goto error;

        // dns
        responses[i].dnses = alloca(32);
        err = syncGetDns(cid, dns1, dns2);
        if (err < 0)
        	goto error;

        if (dns2[0] == '\0')
        	asprintf(&responses[i].dnses, "%s", dns1);
        else
        	asprintf(&responses[i].dnses, "%s %s", dns1, dns2);

        //gageway
        err = syncGetGWNetmask(cid, gw, mask);
        if (err < 0)
        	goto error;
        asprintf(&responses[i].gateways, "%s", gw);

        err = activateNetDev(responses[i].ifname);
        if (err < 0)
            goto error;
    }
...
static void requestSignalStrength(void *data, size_t datalen, RIL_Token t)
{
    ...
    static int response[12] = {
        99, -1,
        -1, -1,
        -1, -1, -1,
        99, 0x7FFFFFFF, 0x7FFFFFFF, 0x7FFFFFFF, 0x7FFFFFFF,
    };
	...
	response[0] = response[0] - 140;

	if (response[0] == -41) {
		response[0] = 0;
	} else if (response[0] >= -51) {
		response[0] = 31;
	} else if ((response[0] >= -109) && (response[0] < -51)) {
		response[0] = (response[0] + 109) / 2 + 2;
	} else if ((response[0] >= -112) && (response[0] < -109)) {
		response[0] = 1;
	} else {
		response[0] = 0;
	}

	RLOGD("requestSignalStrength %d, %d\n", response[0], response[1]);
                                        ...
                                        size_t datalen, RIL_Token t)
{
    ...
    //^SYSINFO:<ServiceStatus>,<ServiceDomain>,<RoamStatus>,<SystemMode>,<SimState>,,<SystemSubMode>
    err = at_tok_nextint(&line, &response[0]);
    if (err < 0) goto error;
    err = at_tok_nextint(&line, &response[1]);
    if (err < 0) goto error;
    ...
    //"^SYSINFO:..." >>> "+CGREG: <stat>[, <lac>, <cid>, <networkType>, ...]"
    if (2 == response[0]) { //modem in service
        if (0 == response[2]) {
            responseStr[0] = "1"; //ServiceState.STATE_IN_SERVICE -- home
        } else {
            responseStr[0] = "5"; //ServiceState.STATE_IN_SERVICE -- roaming
        }
    } else {
        responseStr[0] = "0";
    }
    ...
    responseStr[1] = "-1";
    responseStr[2] = "-1";
    responseStr[3] = "14"; //RIL_RADIO_TECHNOLOGY_LTE
    responseStr[4] = "-1";
    responseStr[5] = "1";
...
static void requestOperator(void *data, size_t datalen, RIL_Token t)
{
    ...
    ATResponse *p_response[3] = {NULL,NULL,NULL};
    ...
    for (i = 0;i<3 ;i=i+2) {// do not support short format

    	sprintf(atcmd,"AT+COPS=3,%d",i);
    	err = at_send_command(atcmd,NULL);// set res format
    	if (err != 0) goto error;

        err = at_send_command_singleline("AT+COPS?","+COPS:", &p_response[i]);

        if (err != 0) goto error;

        p_cur = p_response[i]->p_intermediates;
        if(p_cur == NULL) goto error;
        ...
        //at_response_free(p_response);// free the 1st res ,
    }

    RIL_onRequestComplete(t, RIL_E_SUCCESS, response, sizeof(response));
    at_response_free(p_response[0]);
    at_response_free(p_response[1]);
    at_response_free(p_response[2]);
...
static void requestSetupDataCall(void *data, size_t datalen, RIL_Token t)
{
	...
	err = at_send_command("AT+CGACT=1,1", &p_response);
    ...
    lte_status = LTE_STATUS_ACTIVATED;
    ...
    switch (request) {
        ...
        case RIL_REQUEST_GET_IMSI:
            ...
            if (err < 0 || p_response->success == 0) {
                ...
                size_t prefixLen = strlen("+CIMI: ");
                size_t maxImsiLen = 15;
                if (strlen(p_response->p_intermediates->line) > (prefixLen + maxImsiLen)) {
                    p_response->p_intermediates->line[prefixLen + maxImsiLen] = '\0';
                }
                RIL_onRequestComplete(t, RIL_E_SUCCESS,
                        (p_response->p_intermediates->line + prefixLen),
                        sizeof(char *));
            }
            at_response_free(p_response);
            break;
        ...
        case RIL_REQUEST_GET_IMEI:
            ...
            if (err < 0 || p_response->success == 0) {
                ...
                size_t prefixLen = strlen("+CGSN: ");
				size_t imeiLen = 15;
				if (strlen(p_response->p_intermediates->line) > (prefixLen + imeiLen)) {
					p_response->p_intermediates->line[prefixLen + imeiLen] = '\0';
				}
				RIL_onRequestComplete(t, RIL_E_SUCCESS,
						(p_response->p_intermediates->line + prefixLen),
						sizeof(char *));
        ...
        case RIL_REQUEST_GET_IMEISV:
        	p_response = NULL;
			err = at_send_command_singleline("AT+PRODUCTCFG=readimeisv", "IMEISV: ", &p_response);

			if (err < 0 || p_response->success == 0) {
				RIL_onRequestComplete(t, RIL_E_GENERIC_FAILURE, NULL, 0);
			} else {
				size_t prefixLen = strlen("IMEISV: ");
				size_t imeisvLen = 16;
				if (strlen(p_response->p_intermediates->line) > (prefixLen + imeisvLen)) {
					p_response->p_intermediates->line[prefixLen + imeisvLen] = '\0';
				}
				RIL_onRequestComplete(t, RIL_E_SUCCESS,
						(p_response->p_intermediates->line + prefixLen),
						sizeof(char *));
...
{
    return "android innofidei-ril 1.2";
}
...
setRadioState(RIL_RadioState newState)
{
    ...
    if (sState != oldState) {
         ...
         */
        if (sState == RADIO_STATE_SIM_NOT_READY/*RADIO_STATE_ON*/) {
            onRadioPowerOn();
        }
    }
}
...
#endif /* WORKAROUND_FAKE_CGEV */
    } else if (strStartsWith(s, "+SYSINFO:")) {
		int srv_status;
		//RLOGD("[cai] state change\n");
		RIL_onUnsolicitedResponse (
		            RIL_UNSOL_RESPONSE_VOICE_NETWORK_STATE_CHANGED,
		            NULL, 0);
        ...
        while  (fd < 0) {
            if (s_port > 0) {
                ...
                if ( fd >= 0 ) {
                    ...
                    ios.c_lflag  &= ~(ICANON | ECHO | ISIG);  /*Input*/
                    ios.c_oflag  &= ~OPOST;   /*Output*/

#define BAND_RATE B115200
					/*Set band-rate 115200 8N1 */
					cfsetispeed(&ios, BAND_RATE);
					cfsetospeed(&ios, BAND_RATE);
					ios.c_cflag |= CS8;	/*8-bit data*/
