...
static int is_brother_process(const char *dir, const char *file, void *argv[]) 
{
    ...
    snprintf(filename, MAX_PATH, "%s/%s/fd", dir, file);  
    if (!ls_dir(filename, is_same_linkfile, argv))
        return 0;
    
    dbg_time("%s/%s/exe -> %s", dir, file, myself);
    kill(pid, SIGTERM);
    sleep(1);
    if (!kill(pid, 0))
    {
...
static int kill_brothers(const char *qmichannel)
{
    ...
    return ls_dir("/proc", is_brother_process, argv);
}
...
#define has_more_argv() ((opt < argc) && (argv[opt][0] != '-'))
    ...
    dbg_time("Quectel_ConnectManager_SR01A01V20");
